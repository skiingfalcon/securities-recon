...
A two-layer reconciliation pipeline for the small-fund EOD positions problem in [Instructions.md](Instructions.md). Layer 1 is deterministic (no LLM) and produces a canonical break ledger. Layer 2 is an optional Strands + Bedrock Claude Sonnet agent that investigates each break with mocked OMS / corp-actions tools and emits resolution and escalation artifacts.
...
Static export: [img/architecture.png](img/architecture.png). Solid edges are the deterministic Layer-1 path; dotted edges are the optional Layer-2 agent path.
...
- **Model:** Claude 3.7 Sonnet on Bedrock in `us-west-2` (`us.anthropic.claude-3-7-sonnet-20250219-v1:0`)
...
Layer 2 uses Claude 3.7 Sonnet on Bedrock at the rates baked into `code/run.py`:

```python
# USD per million tokens  Bedrock on-demand pricing, Claude 3.7 Sonnet (us-west-2)
