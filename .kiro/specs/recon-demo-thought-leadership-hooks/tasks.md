...
**Phases 0–3 are complete.** The following files are fully implemented:
...
**Phases 4–9 are pending.** The following are stubs:
- `code/pipeline/reconcile.py` — stub only
...
### Phase 4 — Reconciler (Reqs 1.6, 2.5, 4)

- [ ] 17. Implement cross-custodian join in `code/pipeline/reconcile.py`
...
- [ ] 18. Implement break-type classification
...
- [ ] 19. Implement `identifier_ambiguous` branch and warning attachment
...
- [ ] 20. Attach ingest_warnings to each Break and null the book_* fields
...
- [ ] 21. Verify Reconciler on the case-study inputs
...
### Phase 5 — OutputArtifact envelope (Req 5)

- [ ] 22. Implement input-file SHA-256 helper
...
- [ ] 23. Implement git short-SHA helper with `"uncommitted"` fallback
...
- [ ] 24. Implement `write_envelope` helper
...
- [ ] 25. Wire the envelope into `out/raw_breaks.json` and `out/data_quality.json`
...
- [ ] 26. Verify envelope correctness on a smoke run
...
### Phase 6 — Run_Summary + Layer-1 entrypoint (Req 4 AC 1, 4)

- [ ] 27. Implement Layer-1 `print_run_summary` in `code/pipeline/reconcile.py`
...
- [ ] 28. Wire the `python -m code.pipeline.reconcile` entrypoint
...
- [ ] 29. Verify the Layer-1 entrypoint runs end-to-end
...
- [ ] 30. Write `test_paren_quantity_in_custodian_b_is_parsed_as_short_position`
...
- [ ] 31. Write `test_alphabet_inc_is_ambiguous_between_googl_and_goog_class_shares`
...
- [ ] 32. Write `test_brka_dot_in_ticker_is_preserved_through_normalization`
...
- [ ] 33. Write `test_2025_date_in_2026_eod_file_emits_year_mismatch_warning`
...
- [ ] 34. Write `test_position_only_in_one_custodian_is_classified_as_one_sided`
...
- [ ] 35. Write the suite-level `OutputArtifact` envelope invariant test
...
### Phase 8 — Layer 2 stub: Agent_Runtime + Cost_Reporter (Req 6)

- [ ] 36. (Optional — time-permitting) Build the Strands Agent skeleton in `code/agent.py`
...
- [ ] 37. (Optional — time-permitting) Implement TokenUsage capture and missing-usage warning
...
- [ ] 38. (Optional — time-permitting) Implement Cost_Reporter in `code/run.py`
...
- [ ] 39. (Optional — time-permitting) Wire `python -m code.run` with graceful Layer-1 degradation
...
- [ ] 40. (Optional — time-permitting) Verify the Layer-2 path and its degradation
...
- [ ] 44. (Optional — if Phase 8 done and Bedrock creds present) Run the full demo smoke
  - `uv run python -m code.run`; confirm `out/resolved_breaks.json` and the full cost summary line.
  - _Acceptance:_ Req 6 AC 1, 2
  - _Files:_ (no new files)

- [ ] 45. Author `README.md` from the 12-section structure in #[[file:.kiro/steering/product.md]]
...
- [ ] 46. Export the architecture diagram to `img/architecture.png`
